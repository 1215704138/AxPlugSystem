...
void testTcpServer() {
  ...
  if (started) {
    ...
    std::cout << "监听地址: "
              << (server->GetListenAddress() ? server->GetListenAddress()
                                             : "0.0.0.0")
              << std::endl;
    std::cout << "监听端口: " << server->GetListenPort() << std::endl;
    std::cout << "正在监听: " << (server->IsListening() ? "是" : "否")
              << std::endl;
    std::cout << "正在运行: " << (server->IsRunning() ? "是" : "否")
              << std::endl;
...
void testTcpClient() {
  ...
  std::cout << "缓冲区大小设置为: " << client->GetBufferSize() << " 字节"
            << std::endl;
  ...
  if (connected) {
    ...
    std::cout << "本地地址: "
              << (client->GetLocalAddress() ? client->GetLocalAddress()
                                            : "未知")
              << std::endl;
    std::cout << "本地端口: " << client->GetLocalPort() << std::endl;
    std::cout << "远程地址: "
              << (client->GetRemoteAddress() ? client->GetRemoteAddress()
                                             : "未知")
              << std::endl;
    std::cout << "远程端口: " << client->GetRemotePort() << std::endl;
    std::cout << "连接状态: " << (client->IsConnected() ? "已连接" : "未连接")
              << std::endl;
    ...
    if (binarySent) {
      std::cout << "✅ 二进制数据发送成功，长度: " << sizeof(binaryData)
                << " 字节" << std::endl;
    } else {
    ...
    bool received = client->ReceiveString(receiveBuffer, sizeof(receiveBuffer),
                                          receivedLen);
...
void testServerClientInteraction() {
  ...
  if (serverClient) {
    ...
    char receiveBuffer[1024];
    size_t receivedLen = 0;
    if (serverClient->ReceiveString(receiveBuffer, sizeof(receiveBuffer),
                                    receivedLen)) {
      receiveBuffer[receivedLen] = '\0';
  ...
  if (client->ReceiveString(clientBuffer, sizeof(clientBuffer),
                            clientReceivedLen)) {
...
void testNetworkPerformance() {
  ...
  std::cout << "成功连接: " << successCount << " (预期为0，因为端口不存在)"
            << std::endl;
  std::cout << "总耗时: " << duration.count() << " ms" << std::endl;
  std::cout << "平均每次连接: "
            << static_cast<double>(duration.count()) / testCount << " ms"
            << std::endl;
...
void testBoostNetwork() {
  ...
  if (boostClient) {
    ...
    std::cout << "超时设置: " << boostClient->GetTimeout() << " ms"
              << std::endl;
    std::cout << "缓冲区大小: " << boostClient->GetBufferSize() << " bytes"
              << std::endl;
    std::cout << "KeepAlive: "
              << (boostClient->IsKeepAliveEnabled() ? "启用" : "禁用")
              << std::endl;
    ...
    if (connected) {
      ...
      std::cout << "本地地址: "
                << (boostClient->GetLocalAddress()
                        ? boostClient->GetLocalAddress()
                        : "未知")
                << std::endl;
      std::cout << "本地端口: " << boostClient->GetLocalPort() << std::endl;
      std::cout << "远程地址: "
                << (boostClient->GetRemoteAddress()
                        ? boostClient->GetRemoteAddress()
                        : "未知")
                << std::endl;
      ...
      if (sent) {
        ...
        std::cout << "❌ 数据发送失败: " << boostClient->GetLastError()
                  << std::endl;
      }
      ...
      std::cout << "⚠️ 连接失败 (可能是服务器未启动): "
                << boostClient->GetLastError() << std::endl;
    }

    AxPlug::DestroyTool(boostClient);
  } else {
    std::cout << "❌ Boost TCP客户端创建失败 - 可能是Boost库未正确安装"
              << std::endl;
  }
  ...
  if (boostServer) {
    ...
    std::cout << "最大连接数: " << boostServer->GetMaxConnections()
              << std::endl;
    std::cout << "超时时间: " << boostServer->GetTimeout() << " ms"
              << std::endl;
    ...
    if (started) {
      ...
      std::cout << "监听地址: "
                << (boostServer->GetListenAddress()
                        ? boostServer->GetListenAddress()
                        : "0.0.0.0")
                << std::endl;
      std::cout << "正在监听: " << (boostServer->IsListening() ? "是" : "否")
                << std::endl;
      std::cout << "正在运行: " << (boostServer->IsRunning() ? "是" : "否")
                << std::endl;
      ...
      // 检查连接数
      std::cout << "当前连接数: " << boostServer->GetConnectedCount()
                << std::endl;
      ...
      std::cout << "❌ Boost TCP服务器启动失败: " << boostServer->GetLastError()
                << std::endl;
    }

    AxPlug::DestroyTool(boostServer);
  } else {
    std::cout << "❌ Boost TCP服务器创建失败 - 可能是Boost库未正确安装"
              << std::endl;
  }
}
