...
LoggerService::LoggerService()
    : currentLevel_(LogLevel::Info), logFilePath_("logs/app.log") // Modified to logs directory
      ,
      consoleOutputEnabled_(true) // Kept original name, assuming no other changes to this member
      ,
      timestampFormat_("default"), fileOpen_(false), asyncEnabled_(false), stopFlag_(false)
{
...
void LoggerService::formatLogMessage(LogLevel level, const std::string& message, std::string& formattedMessage)
{
    std::ostringstream oss;
...
void LoggerService::Info(const char* message)
{
    Log(LogLevel::Info, message);
}

void LoggerService::Warn(const char* message)
{
...
bool LoggerService::IsAsyncLoggingEnabled() const
{
...
void LoggerService::asyncLogWorker()
{
    while (true)
    {
        ...
        // 等待日志消息或停止信号
        queueCondition_.wait(lock, [this] { return !logQueue_.empty() || stopFlag_; });
