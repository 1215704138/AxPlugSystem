...
UdpSocket::UdpSocket()
    : socket_(INVALID_SOCKET), isBound_(false), timeout_(5000),
      bufferSize_(4096), ttl_(64), broadcastEnabled_(false), localPort_(0),
      errorCode_(0) {}

UdpSocket::~UdpSocket() { Unbind(); }
...
bool UdpSocket::setSocketOptions() {
  // 设置超时
  if (timeout_ > 0) {
    setsockopt(socket_, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout_,
               sizeof(timeout_));
    setsockopt(socket_, SOL_SOCKET, SO_SNDTIMEO, (char *)&timeout_,
               sizeof(timeout_));
  }

  // 设置缓冲区大小
  if (bufferSize_ > 0) {
    setsockopt(socket_, SOL_SOCKET, SO_RCVBUF, (char *)&bufferSize_,
               sizeof(bufferSize_));
    setsockopt(socket_, SOL_SOCKET, SO_SNDBUF, (char *)&bufferSize_,
               sizeof(bufferSize_));
  }
...
bool UdpSocket::Bind(int port) {
  ...
  if (bind(socket_, (sockaddr *)&localAddr, sizeof(localAddr)) ==
      SOCKET_ERROR) {
...
bool UdpSocket::IsBound() const { return isBound_; }
...
bool UdpSocket::SendTo(const char *host, int port, const uint8_t *data,
                       size_t len) {
  if (socket_ == INVALID_SOCKET) {
    setError("socket未创建", 0);
    return false;
...
bool UdpSocket::Receive(uint8_t *buffer, size_t maxLen, size_t &outLen) {
  ...
  int received = recvfrom(socket_, (char *)buffer, static_cast<int>(maxLen), 0,
...
bool UdpSocket::ReceiveFrom(char *host, int hostLen, int &port, uint8_t *buffer,
...
bool UdpSocket::ReceiveStringFrom(char *host, int hostLen, int &port,
                                  char *buffer, size_t maxLen, size_t &outLen) {
  return ReceiveFrom(host, hostLen, port, reinterpret_cast<uint8_t *>(buffer),
                     maxLen, outLen);
}
...
bool UdpSocket::IsBroadcastEnabled() const { return broadcastEnabled_; }
...
bool UdpSocket::GetMulticastGroups(char *groups, int maxLen, int &count) const {
  ...
  for (const auto &group : multicastGroups_) {
    int groupLen =
        static_cast<int>(group.length()) + 1; // +1 for null terminator
...
const char *UdpSocket::GetLocalAddress() const { return localAddr_.c_str(); }

int UdpSocket::GetLocalPort() const { return localPort_; }
...
int UdpSocket::GetTimeout() const { return timeout_; }
...
int UdpSocket::GetBufferSize() const { return bufferSize_; }
...
int UdpSocket::GetTTL() const { return ttl_; }

const char *UdpSocket::GetLastError() const { return lastError_.c_str(); }

int UdpSocket::GetErrorCode() const { return errorCode_; }
